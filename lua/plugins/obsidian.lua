return {
  ...
  opts = {
        ...
        vim.keymap.set('n', '<leader>nn', function()
          ...
          vim.ui.input({ prompt = 'Enter note title: ' }, function(title)
            ...
            local note = Note.create {
              id = title, -- Pass title as id for note_id_func to transform
              aliases = { title }, -- Add original title as alias
            ...
            -- Add H1 header (no longer auto-generated by obsidian.nvim)
            local bufnr = vim.api.nvim_get_current_buf()
            local line_count = vim.api.nvim_buf_line_count(bufnr)
            vim.api.nvim_buf_set_lines(bufnr, line_count, line_count, false, { '', '# ' .. title })
          end)
        end, {
          buffer = ev.buf,
    ...
    -- Optional, configure frontmatter handling
    frontmatter = {
      enabled = true,
      func = function(note)
        -- Alias is now passed via Note.create, no need to add here
